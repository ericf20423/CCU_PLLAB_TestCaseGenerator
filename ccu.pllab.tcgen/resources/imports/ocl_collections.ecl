...
ocl_set_asSet(Set, Result) :- 
   ( is_collection(Set) -> Result = Set; Result = [Set]).
...
ocl_set_asOrderedSet(Set, Result) :- 
   ( is_collection(Set) -> sort(Set, Result); Result = [Set] ).
...
ocl_bag_asSet(Bag, Result) :- 
   ( is_collection(Bag) -> sort(Bag, Result); Result = [Bag]).
...
ocl_bag_asSequence(Bag, Result) :- 
   ( is_collection(Bag) -> Result = Bag; Result = [Bag] ).
...
% Operations inherited from collection
ocl_sequence_size(Seq, Size)               :- ocl_col_size(Seq, Size).
ocl_sequence_includes(Seq, Obj, Result)    :- ocl_col_includes(Seq, Obj, Result).
ocl_sequence_excludes(Seq, Obj, Result)    :- ocl_col_excludes(Seq, Obj, Result).
ocl_sequence_count(Seq, Obj, N)            :- ocl_col_count(Seq, Obj, N).
ocl_sequence_includesAll(Seq, Col, Result) :- ocl_col_includesAll(Seq, Col, Result).
ocl_sequence_excludesAll(Seq, Col, Result) :- ocl_col_excludesAll(Seq, Col, Result).
ocl_sequence_isEmpty(Seq, Result)          :- ocl_col_isEmpty(Seq, Result).
ocl_sequence_notEmpty(Seq, Result)         :- ocl_col_notEmpty(Seq, Result).
ocl_sequence_sum(Seq, N)                   :- ocl_col_sum(Seq, N).
ocl_sequence_product(Seq, Col, Product)    :- ocl_col_product(Seq, Col, Product).        

% ocl_sequence_equals( +Seq1, +Seq2, -Result ) :-
%    Result is 1 if Seq1 and Seq2 are equal (same elements and same order)
%    and 0 otherwise

delay ocl_sequence_equals(X, _, _) if nonground(X).
delay ocl_sequence_equals(_, Y, _) if nonground(Y).
ocl_sequence_equals(Seq1, Seq2, Result) :- 
   ( Seq1 = Seq2 -> Result = 1; Result = 0).
 
% ocl_sequence_union( +Seq1, +Seq2, -Result ) :-
%    Result is the sequence formed by all the elements of Seq1 followed by
%    the elements of Seqs

delay ocl_sequence_union(X, _, _) if var(X).
delay ocl_sequence_union(_, Y, _) if var(Y).
ocl_sequence_union(Seq1, Seq2, Result) :- 
   append(Seq1, Seq2, Result).

% ocl_sequence_flatten( +Seq, -Result ) :- 
...
delay ocl_sequence_flatten(X, _) if nonground(X).
ocl_sequence_flatten(Seq, Result) :- 
   flatten(Seq, Result). 

% ocl_sequence_append( +Seq, +Obj, -Result ) :- 
%    Result is the sequence computed by adding Obj at the end of Seq

delay ocl_sequence_append(X, _, _) if var(X).
ocl_sequence_append(Seq, Obj, Result) :- 
   append( Seq, [Obj], Result ).

% ocl_sequence_prepend( +Seq, +Obj, -Result ) :- 
%    Result is the sequence computed by adding Obj at the beginning of Seq

delay ocl_sequence_prepend(X, _, _) if var(X).
ocl_sequence_prepend(Seq, Obj, Result) :- 
   Result = [Obj|Seq].

% ocl_sequence_insertAt( +Seq, +Obj, +Idx, -Result ) :- 
...
delay ocl_sequence_insertAt(X, _, _, _) if var(X).
delay ocl_sequence_insertAt(_, _, Y, _) if nonground(Y).
ocl_sequence_insertAt([], Obj, _, Result)  :-        
       ocl_mustBeObject(Obj, Object),
       Result = [Object], !.
ocl_sequence_insertAt(Seq, Obj, 1, Result) :- 
       ocl_mustBeObject(Obj, Object),
       Result = [Object|Seq], !.
ocl_sequence_insertAt([First|Rest],Obj,X,Result) :-
       ocl_mustBeObject(Obj, Object),
       X > 1, !,
       N is X - 1,
       ocl_sequence_insertAt(Rest, Object, N, Aux),
       Result = [First|Aux].

% ocl_sequence_subSequence( +Seq, +Lower, +Upper, -Result ) :- 
...
delay ocl_sequence_subSequence(X, _, _, _) if var(X).
delay ocl_sequence_subSequence(_, Y, _, _) if nonground(Y).
delay ocl_sequence_subSequence(_, _, Z, _) if nonground(Z).
ocl_sequence_subSequence(Seq, X, X, Result) :-   
       !,
       nth1(X, Seq, Aux),
       Result = [Aux].
ocl_sequence_subSequence(Seq, Lower, Upper, Result) :-
       Lower < Upper, !,
       N is Lower + 1,
       ocl_sequence_subSequence(Seq, N, Upper, AuxList),
...
% ocl_sequence_ at( +Seq, +Idx, -Result ) :- 
%    Result is the element within Seq in position Idx

delay ocl_sequence_at(X, _, _) if var(X).
delay ocl_sequence_at(_, Y, _) if nonground(Y).
ocl_sequence_at(Seq, Idx, Result) :- 
   nth1(Idx, Seq, Result).

% ocl_sequence_indexOf( +Seq, +Obj, -Result ) :- 
...
delay ocl_sequence_indexOf(X, _, _) if nonground(X).
delay ocl_sequence_indexOf(_, Y, _) if nonground(Y).
ocl_sequence_indexOf( [], _, _ ) :- fail.
ocl_sequence_indexOf( [Object|_], Obj, 1) :- 
       ocl_mustBeObject(Obj, Object),
       !.
ocl_sequence_indexOf( [First|Rest], Obj, Result) :- 
       ocl_mustBeObject(Obj, Object),
       First \= Object, !,
       ocl_sequence_indexOf( Rest, Obj, N ),
       Result is N + 1.
                
% ocl_sequence_first( +Seq, -Result ) :- 
%    Return the first element in the sequence SEQ
%    Behavior is not specified if the sequence is empty

delay ocl_sequence_first(X, _) if var(X).
ocl_sequence_first([], _) :- fail.
ocl_sequence_first([X|_], X) :- !.

% ocl_sequence_last( +Seq, -Result ) :- 
%    Return the last element in the sequence SEQ
%    Behavior is not specified if the sequence is empty

delay ocl_sequence_last(X, _) if var(X).
ocl_sequence_last([], _ ) :- fail.
ocl_sequence_last([First|Rest], Result) :-
   length([First|Rest], N),
   nth1([First|Rest], N, Result).
       
% ocl_sequence_including( +Seq, +Obj, -Result ) :- 
%   Result is the sequence computed by adding Obj at the end of Seq
%   This operation is equivalent to append()

ocl_sequence_including(Seq, Obj, Result) :- 
   ocl_mustBeObject(Obj, Object),
   ocl_sequence_append(Seq, Object, Result).

% ocl_sequence_excluding( +Seq, +Obj, -Result ) :- 
...
% ocl_sequence_asSet( +Seq, -Result ) :- 
%    Return a Set view of this sequence

delay ocl_sequence_asSet(X, _) if nonground(X).
ocl_sequence_asSet(Seq, Result) :- 
   ( is_collection(Seq) -> list_to_ord_set(Seq, Result); Result = [Seq] ).

% ocl_sequence_asSequence( +Seq, -Result ) :- 
%    Return the same sequence

delay ocl_sequence_asSequence(X, _) if var(X).
ocl_sequence_asSequence(Seq, Result) :- 
   ( is_collection(Seq) -> Result = Seq; Result = [Seq] ).

% ocl_sequence_asBag( +Seq, -Result ) :- 
%    Return a Bag view of this sequence

delay ocl_sequence_asBag(X, _) if var(X).
ocl_sequence_asBag(Seq, Result) :- 
   ( is_collection(Seq) -> Result = Seq; Result = [Seq] ).

% ocl_sequence_asOrderedSet( +Seq, -Result ) :- 
...
delay ocl_sequence_asOrderedSet(X, _) if nonground(X).
ocl_sequence_asOrderedSet([], []) :- !.
ocl_sequence_asOrderedSet(X, [X]) :- not is_collection(X), !.
ocl_sequence_asOrderedSet([First|Rest],Result) :-
   ocl_sequence_asOrderedSet(Rest, Aux),
   ( ocl_sequence_includes(Rest, First, 1) ->
     Result = Aux; 
     Result = [First|Aux] 
   ).
...
% Operations inherited from collection
ocl_boolean_ordset_size(Set, Size)            :- ocl_col_size(Set, Size).
ocl_boolean_ordset_includes(Set, Obj)         :- ocl_col_includes(Set, Obj).
ocl_boolean_ordset_excludes(Set, Obj)         :- ocl_col_excludes(Set, Obj).
ocl_boolean_ordset_count(Set, Obj, N)         :- ocl_col_count(Set, Obj, N).
ocl_boolean_ordset_includesAll(Set, Col, Result) :- 
                                      ocl_col_includesAll(Set, Col, Result).
ocl_boolean_ordset_excludesAll(Set, Col, Result) :- 
                                      ocl_col_excludesAll(Set, Col, Result).
ocl_boolean_ordset_isEmpty(Set, Result)       :- ocl_col_isEmpty(Set, Result).
ocl_boolean_ordset_notEmpty(Set, Result)      :- ocl_col_notEmpty(Set, Result).
ocl_boolean_ordset_sum(Set, N)                :- ocl_col_sum(Set, N).
ocl_boolean_ordset_product(Set, Col, Product) :- ocl_col_product(Set, Col, Product).

% ocl_boolean_ordset_append( +Ordset, +Obj, -Result) :- 
%    If Obj is not in Ordset, Result has all the elements of OrdSet plus Obj
%    at the end. Otherwise, Result is equal to OrdSet

delay ocl_boolean_ordset_append(X,_,_) if nonground(X).
delay ocl_boolean_ordset_append(_,Y,_) if nonground(Y).
ocl_boolean_ordset_append(OrdSet, Obj, Result) :- 
   ocl_mustBeObject(Obj, Object),
   ( ocl_boolean_ordset_includes(OrdSet, Object, 1) ->
     Result = OrdSet;
     append(OrdSet, [Object], Result)
   ).

% ocl_boolean_ordset_prepend( +OrdSet, +Obj, -Result ) :- 
%    If Obj is not in Ordset, Result has all the elements of OrdSet plus Obj
%    at the beginning. Otherwise, Result is equal to OrdSet

delay ocl_boolean_ordset_prepend(X,_,_) if nonground(X).
delay ocl_boolean_ordset_prepend(_,Y,_) if nonground(Y).
ocl_boolean_ordset_prepend(OrdSet, Obj, Result) :- 
   ocl_mustBeObject(Obj, Object),
   ( ocl_boolean_ordset_includes(OrdSet, Object, 1) ->
     Result = OrdSet;
...
% ocl_boolean_ordset_insertAt( +OrdSet, +Obj, +Idx, -Result) :- 
%    If Obj is in OrdSet, Result is equal to OrdSet
%    Otherwise, Result is computed by inserting Obj in position Idx of OrdSet

delay ocl_boolean_ordset_insertAt(X, _, _, _) if nonground(X).
delay ocl_boolean_ordset_insertAt(_, Y, _, _) if nonground(Y).
delay ocl_boolean_ordset_insertAt(_, _, Z, _) if nonground(Z).
ocl_boolean_ordset_insertAt(OrdSet, Obj, Idx, Result) :-
   ocl_mustBeObject(Obj, Object),
   ( ocl_boolean_ordset_inclues(OrdSet, Object, 1) ->
     Result = OrdSet;
     ocl_sequence_insertAt(OrdSet, Object, Idx, Result)
   ).

% ocl_boolean_ordset_subOrderedSet( +OrdSet, +Lower, +Upper, -Result ) :-
%    Returns the subset between positions Lower and Upper within the set
%    According to the precondition 1 <= Lower <= Upper <= size(Seq)

ocl_boolean_ordset_subOrderedSet(OrdSet, Lower, Upper, Result) :-
   ocl_sequence_subSequence(OrdSet, Lower, Upper, Result).

% ocl_boolean_ordset_at( +OrdSet, +Idx, -Result ) :- 
%    Return the element with position IDX within the set OrdSet
%    According to the precondition, 1 <= Idx <= size(Seq)

ocl_boolean_ordset_at(OrdSet, Idx, Result) :-
   ocl_sequence_at(OrdSet, Idx, Result).
       
% ocl_boolean_ordset_first( +OrdSet, -Result ) :- 
%    Return the first element of the ordered set

ocl_boolean_ordset_first(OrdSet, Result) :-       
   ocl_sequence_first(OrdSet, Result).
       
% ocl_boolean_ordset_last( +OrdSet, +Result ) :- 
%    Return the last element of the ordered set
ocl_boolean_ordset_last(OrdSet, Result) :-
   ocl_sequence_last(OrdSet, Result).
