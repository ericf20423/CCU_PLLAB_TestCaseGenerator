...
orderForAttributes(ObjectA, ObjectB) :-
  ( foreach(AttribA, ObjectA), 
    foreach(AttribB, ObjectB),
    fromto(0, InOrder, OutOrder, AllLess),
    fromto(1, InEquals, OutEquals, AllEqual)
    do     
      (
	(not number(AttribA), not number(AttribB) ->
         (
	    Less #= 0,
	    Equals #= 1
	 );(
...
differenceForLinks(LinkA, LinkB) :-
  ( foreach(ParticipantA, LinkA),
    foreach(ParticipantB, LinkB),
    fromto(0, In, Out, Result)
    do
       ((not number(ParticipantA), not number(ParticipantB)) ->
       Aux = 1;
...
orderForLinks(LinkA, LinkB) :-
  ( foreach(ParticipantA, LinkA), 
    ...
    do
      not number(ParticipantA), not number(ParticipantB) -> true;
